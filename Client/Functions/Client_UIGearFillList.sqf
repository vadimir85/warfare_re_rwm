	...
	if !(isNil '_get') then {
		if ((_get select QUERYGEARUPGRADE) <= _currentGearUpgr) then {
			...
			if (_add) then {
				...
				(findDisplay 16000 displayCtrl _listBox) lbSetTooltip [_i*2, _x];
				_list1 = _list1 + [_x];
				_i = _i + 1;
			};
		};
...
{if (_x == _currentItem) then {lnbSetCurSelRow [_listBox, _forEachIndex]}} forEach _list1;