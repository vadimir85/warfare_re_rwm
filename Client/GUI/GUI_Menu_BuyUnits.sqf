...
while {alive player && dialog} do {
	...
	//--- Tabs selection.
	if (Warfare_MenuAction == 101) then {Warfare_MenuAction = -1;if (barracksInRange) then {_currentIDC = 12005;_type = 'Barracks';_val = 0;_update = true}};
	if (Warfare_MenuAction == 102) then {Warfare_MenuAction = -1;if (lightInRange) then {_currentIDC = 12006;_type = 'Light';_val = 1;_update = true}};
	if (Warfare_MenuAction == 103) then {Warfare_MenuAction = -1;if (heavyInRange) then {_currentIDC = 12007;_type = 'Heavy';_val = 2;_update = true}};
	if (Warfare_MenuAction == 104) then {Warfare_MenuAction = -1;if (aircraftInRange) then {_currentIDC = 12008;_type = 'Aircraft';_val = 3;_update = true}};
	if (Warfare_MenuAction == 105) then {Warfare_MenuAction = -1;if (DepotInRange) then {_currentIDC = 12020;_type = 'Depot';_val = 4;_update = true}};
	if (Warfare_MenuAction == 106) then {Warfare_MenuAction = -1;if (hangarInRange) then {_currentIDC = 12021;_type = 'Airport';_val = 3;_update = true}};
	
	//--- driver-gunner-commander icons.
	if (Warfare_MenuAction == 201) then {Warfare_MenuAction = -1;_driver = if (_driver) then {false} else {true};_updateDetails = true};
	if (Warfare_MenuAction == 202) then {Warfare_MenuAction = -1;_gunner = if (_gunner) then {false} else {true};_updateDetails = true};
	if (Warfare_MenuAction == 203) then {Warfare_MenuAction = -1;_commander = if (_commander) then {false} else {true};_updateDetails = true};
	if (Warfare_MenuAction == 204) then {Warfare_MenuAction = -1;_extracrew = if (_extracrew) then {false} else {true};_updateDetails = true};
	
	//--- Factory DropDown list value has changed.
	if (Warfare_MenuAction == 301) then {Warfare_MenuAction = -1;_factSel = lbCurSel 12018;_closest = _sorted select _factSel;_updateMap = true};
	
	//--- Selection change, we update the details.
	if (Warfare_MenuAction == 302) then {Warfare_MenuAction = -1;_updateDetails = true};
	
	//--- Faction Filter changed.
	if (Warfare_MenuAction == 303) then {Warfare_MenuAction = -1;_update = true;[Format["WFBE_%1%2CURRENTFACTIONSELECTED",sideJoinedText,_type],(lbCurSel _comboFaction),true] Call SetNamespace};
	
	//--- Lock icon.
	if (Warfare_MenuAction == 401) then {Warfare_MenuAction = -1;_isLocked = if (_isLocked) then {false} else {true};_updateDetails = true};
