...
while {alive player && dialog} do {
	...
	if (Warfare_MenuAction == 601) then {if (_mode != 0) then {_updateTab = true};_mode = 0};//--- Team properties.
	if (Warfare_MenuAction == 602) then {if (_mode != 1) then {_updateTab = true};_mode = 1};//--- Task.
	if (Warfare_MenuAction == 603) then {if (_mode != 2) then {_updateTab = true};_mode = 2};//--- Task.
	...
	if (Warfare_MenuAction == 1 and paramAI) then {
		Warfare_MenuAction = -1;
	...
	//--- Take Towns.
	if (Warfare_MenuAction == 101) then {
		Warfare_MenuAction = -1;
		if (!_isAll) then {
	...
	if (mouseButtonUp == 0) then {
		mouseButtonUp = -1;
		//--- Move | Patrol | Defend.
		if (Warfare_MenuAction == 102 || Warfare_MenuAction == 103 || Warfare_MenuAction == 104) then {
			_color = "";
			_order = "";
			_isAdded = "";
			switch (Warfare_MenuAction) do {
		...
		//--- Set a Task.
		if (Warfare_MenuAction == 306) then {
			Warfare_MenuAction = -1;
	...
	//--- Respawn.
	if (Warfare_MenuAction == 201) then {
		Warfare_MenuAction = -1;
		[_team] Spawn {
			Private ["_team"];
	...
	//--- Auto AI.
	if (Warfare_MenuAction == 301) then {
		Warfare_MenuAction = -1;
		_isDisabled = (_team) Call GetTeamAutonomous;
		if (_isDisabled) then {
			ctrlEnable [14015,true];
	...
	//--- Set buy Type
	if (Warfare_MenuAction == 302) then {
		Warfare_MenuAction = -1;
	...
	if (Warfare_MenuAction == 303) then {
		Warfare_MenuAction = -1;
	...
	//--- Set respawn.
	if (Warfare_MenuAction == 304) then {
		Warfare_MenuAction = -1;
	...
	if (Warfare_MenuAction == 305 && _updateProperties) then {Warfare_MenuAction = -1};
	if (Warfare_MenuAction == 305) then {
		Warfare_MenuAction = -1;
		_curSel = lbCurSel 14025;
		if (_curSel == -1) then {_curSel = 0};
	...
	//--- Units Details.
	if (Warfare_MenuAction == 401) then {
		Warfare_MenuAction = -1;
		_iddx = lnbCurSelRow 14041;
		if (_iddx != -1) then {
	...
	if (Warfare_MenuAction == 402) then {
		Warfare_MenuAction = -1;
		_iddx = lnbCurSelRow 14041;
		if (_iddx != -1) then {
			_unit = _detailGroup select _iddx;
	...
	if (Warfare_MenuAction == 403) then {
		Warfare_MenuAction = -1;
		_iddx = lnbCurSelRow 14041;
		if (_iddx != -1) then {
			_unit = _detailGroup select _iddx;
	...
	//--- Back Button.
	if (Warfare_MenuAction == 4) exitWith { //---added-MrNiceGuy
		Warfare_MenuAction = -1;
		closeDialog 0;
