...
_type = if (difficultyOption "thirdPersonView" == 0) then {["Internal"]} else {["Internal","External","Ironsight","Group"]};
...
while {true} do {
	...
	//--- Leader commands AIs.
	if (Warfare_MenuAction == 102) then {
		Warfare_MenuAction = -1;
	...
	//--- Camera Modes
	if (Warfare_MenuAction == 103) then {
		Warfare_MenuAction = -1;
	...
	if (Warfare_MenuAction == 140 && !(isNil "_currentUnit")) then {
		Warfare_MenuAction = -1;
