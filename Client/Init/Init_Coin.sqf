...
if (count _defenses > 0) then {if (_extra == "REPAIR") then {_coinCategories = []};_coinCategories = _coinCategories + [localize "strwfdefend"] + [localize "STR_WF_Fortification"] + [localize "STR_WF_Strategic"] + [localize "STR_WF_UPGRADE_Supply"] + [localize "STR_COIN_EXIT2"]};
...
for '_i' from 0 to count(_defenses)-1 do {
	...
	switch (_defenseCategories select _i) do {
		...
		case "Supply": {_curId = _indexCategory + 3};		
		case "Ammo": {_curId = _indexCategory + 4};
	};
