	...
	if !(_killed in [('WFBE_WESTMHQNAME' Call GetNamespace),('WFBE_EASTMHQNAME' Call GetNamespace)]) then {
		if !(_objectType in [Format ["WFBE_%1FARP",WEST] Call GetNamespace,Format ["WFBE_%1FARP",EAST] Call GetNamespace]) then {
			if (isServer) then {
				...
				_killed unassignItem "I_UavTerminal";
				_killed Spawn TrashObject;
			};
		};
	};
		...
		if !(_iskillerplayer) then {
			[_objectType,leader _killerTeam] Spawn {
				...
				_point = switch (true) do {
					...
					case (_type isKindOf "Ship"): {6};
					case (_type isKindOf "Motorcycle"): {2};
					case (_type isKindOf "Tank"): {6};
					case (_type isKindOf "Helicopter"): {6};
					case (_type isKindOf "Plane"): {10};
					case (_type isKindOf "StaticWeapon"): {1};
