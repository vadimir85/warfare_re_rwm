diag_log Format["[WFRE (INIT)][frameno:%2 | ticktime:%3] Init_Common: Init Start at %1",time,diag_frameno,diag_tickTime];
...
//WFBE_CO_FNC_GetClosestEntity = Compile preprocessFile "Common\Functions\Common_GetClosestEntity.sqf";
...
if (paramICBM) then {
	...
	PostNuclearEffects = Compile preprocessFile "Client\Module\Nuke\post_nuclear_effects.sqf";
};

diag_log Format["[WFRE (INIT)][frameno:%1 | ticktime:%2] Init_Common: Functions - [Done]",diag_frameno,diag_tickTime];
...
/* Starting Supply */
if (isNil "EastSupplies") then {EastSupplies = if (WF_Debug) then {900000} else {120000}};
if (isNil "WestSupplies") then {WestSupplies = if (WF_Debug) then {900000} else {120000}};

diag_log Format["[WFRE (INIT)][frameno:%3 | ticktime:%4] Init_Common: Starting Supply (West: %1 East: %2) - [Done]",WestSupplies,EastSupplies,diag_frameno,diag_tickTime];

/* Starting Money */
['WFBE_EASTSTARTINGMONEY',if (WF_Debug) then {900000} else {800000},false] Call SetNamespace;
['WFBE_WESTSTARTINGMONEY',if (WF_Debug) then {900000} else {800000},false] Call SetNamespace;

diag_log Format["[WFRE (INIT)][frameno:%3 | ticktime:%4] Init_Common: Starting Funds (West: %1 East: %2) - [Done]",'WFBE_WESTSTARTINGMONEY' Call GetNamespace,'WFBE_EASTSTARTINGMONEY' Call GetNamespace,diag_frameno,diag_tickTime];
...
if (('WFBE_ALLIES' Call GetNamespace) > 0) then {
	...
	diag_log Format["[WFRE (INIT)][frameno:%3 | ticktime:%4] Init_Common: Allies Starting Funds (West: %1 East: %2) - [Done]",('WFBE_WESTSTARTINGMONEY' Call GetNamespace)*5,('WFBE_EASTSTARTINGMONEY' Call GetNamespace)*5,diag_frameno,diag_tickTime];
};
...
*/
		/* Gear Core */
		[] Call Compile preprocessFile 'Common\Config\Core_Gear\Core_Default.sqf';
		[] Call Compile preprocessFile 'Common\Config\Core_Gear\Core_RHS.sqf';
		/* Class Core */
		[] Call Compile preprocessFile 'Common\Config\Core\Core_RHS_CDF.sqf';
		[] Call Compile preprocessFile 'Common\Config\Core\Core_RHS_ChDKZ.sqf';
		[] Call Compile preprocessFile 'Common\Config\Core\Core_RHS_CIV.sqf';		
		[] Call Compile preprocessFile 'Common\Config\Core\Core_RHS_GUE.sqf';
		[] Call Compile preprocessFile 'Common\Config\Core\Core_RHS_RU.sqf';		
		[] Call Compile preprocessFile 'Common\Config\Core\Core_RHS_RU_MVD.sqf';	
		[] Call Compile preprocessFile 'Common\Config\Core\Core_RHS_US.sqf';
		[] Call Compile preprocessFile 'Common\Config\Core\Core_RHS_USMC.sqf';
...
diag_log Format["[WFRE (INIT)][frameno:%1 | ticktime:%2] Init_Common: Core Loading - [Done]",diag_frameno,diag_tickTime];
...
diag_log Format["[WFRE (INIT)][frameno:%1 | ticktime:%2] Init_Common: Config Loading - [Done]",diag_frameno,diag_tickTime];
...
diag_log Format["[WFRE (INIT)][frameno:%1 | ticktime:%2] Init_Common: Boundaries Loading - [Done]",diag_frameno,diag_tickTime];
...
diag_log Format["[WFRE (INIT)][frameno:%2 | ticktime:%3] Init_Common: Init End at %1",time,diag_frameno,diag_tickTime];
