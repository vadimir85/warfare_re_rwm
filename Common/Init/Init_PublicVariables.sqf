...
if (local player) then {
	//Client "CanJoin" call gereric error :(
	//{Format['WFBE_%1',_x] addPublicVariableEventHandler {(_this select 1) Spawn HandlePVF}} forEach _clientCommandPV;
	{if !(_x == "CanJoin") then {Format['WFBE_%1',_x] addPublicVariableEventHandler {(_this select 1) Spawn HandlePVF}}} forEach _clientCommandPV;
};
//--- Handle the server side publicVariable.
if (isServer) then {
	//Server "RequestJoin" call gereric error :(
	//{Format['WFBE_%1',_x] addPublicVariableEventHandler {(_this select 1) Spawn HandleSPVF}} forEach _serverCommandPV;
	{if !(_x == "RequestJoin") then {Format['WFBE_%1',_x] addPublicVariableEventHandler {(_this select 1) Spawn HandleSPVF}}} forEach _serverCommandPV;
};

diag_log Format["[WFRE (INIT)][frameno:%3 | ticktime:%4] Init_PublicVariables: Public Variable Initialization (SRV: %1 CLT: %2) - [Done]",count _serverCommandPV, count _clientCommandPV,diag_frameno,diag_tickTime];