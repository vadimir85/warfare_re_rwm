Private ["_areas","_deployed","_direction","_grp","_HQ","_HQName","_logic","_MHQ","_near","_position","_side","_sideText","_site","_text","_type","_update","_varTH","_color"];
...
if (count (crew _HQ) > 0) then {{_x action ["Eject", _HQ]} forEach crew _HQ;sleep 3;};

if (!_deployed) then {
	_HQ setPos [6400,6400,1];
	...
	//ProcessInitCommands;
	[[[_site,true,_side], "Client\Init\Init_BaseStructure.sqf"], "BIS_fnc_execVM", true, true] call BIS_fnc_MP;
	...
	diag_log Format["[WFRE (INFORMATION)][frameno:%3 | ticktime:%4] Construction_HQSite: The %1 MHQ (%2) was deployed",_sideText,_type,diag_frameno,diag_tickTime];
	deleteVehicle _HQ;
} else {
	...
	if (_side == WEST) then {_color = "ColorBlue"} else {_color = "ColorRed"};
	[[['n_hq',_color,[1,1],'HQ','HQUndeployed',_MHQ,0.2,true,'n_hq','ColorOrange',true,_side], "Common\Common_MarkerUpdate.sqf"], "BIS_fnc_execVM", true, true] call BIS_fnc_MP;
	
	diag_log Format["[WFRE (INFORMATION)][frameno:%3 | ticktime:%4] Construction_HQSite: The %1 MHQ (%2) was mobilized",_sideText,_HQName,diag_frameno,diag_tickTime];
