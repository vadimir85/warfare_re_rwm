...
if ((typeOf _defense) in (Format['WFBE_%1SUPPLYTRUCKS',str _side] Call GetNamespace)) then {[[[_defense,_side], "Common\Common_InitUnit.sqf"], "BIS_fnc_execVM", true, true] call BIS_fnc_MP; [_defense,_side] ExecFSM 'Client\FSM\updatesupply.fsm'};

diag_log Format["[WFRE (INFORMATION)][frameno:%3 | ticktime:%4] Construction_StationaryDefense: A %1 %2 was created",str _side,_type,diag_frameno,diag_tickTime];

//--- If it's a minefield, we exit the script while spawning it.
if (_type == 'Land_Sign_Mines_F') exitWith {
	Private ["_c","_h","_mine","_mineType","_toWorld"];
	_mineType = if (_side == east) then {'APERSMine'} else {'APERSMine'};
	_h = -4;
	_c = 0;
...
*/
if (_type == 'Land_SM_01_shelter_wide_F') then {
...
if (paramArtyUI) then {
	...
	if (_isVeh == 1 || _isAC == 1) then {
		...
		[[[_defense], "Common\Common_InitArtillery.sqf"], "BIS_fnc_execVM", true, true] call BIS_fnc_MP;
		//ProcessInitCommands;
		diag_log Format["[WFRE (INFORMATION)][frameno:%3 | ticktime:%4] Construction_StationaryDefense: Artillery UI has been set over the %1 %2",str _side,_type,diag_frameno,diag_tickTime];
	};
};
