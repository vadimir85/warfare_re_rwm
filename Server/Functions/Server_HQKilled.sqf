...
diag_log Format["[WFRE (INFORMATION)][frameno:%2 | ticktime:%3] Server_HQKilled: The %1 MHQ has been destroyed",str _side,diag_frameno,diag_tickTime];
...
if ((side _killer == _side)&&(isPlayer(_killer))) then {
	...
	diag_log Format["[WFRE (INFORMATION)][frameno:%3 | ticktime:%4] Server_HQKilled: Player %1 (%2) has teamkilled the MHQ.",name _killer,_uid,diag_frameno,diag_tickTime];
};
_text="";
sleep random(2);
if (_deployed) then {
	_HQName = Format["WFBE_%1MHQNAME",_side] Call GetNamespace;
	_pos2 = [(_pos getPos [20 * sqrt random 1, random 360] select 0),(_pos getPos [20 * sqrt random 1, random 360] select 1),5];
	_MHQ = _HQName createVehicle _pos2;
	
	_MHQ setDir _dir;
	_MHQ setDammage 1;
	Call Compile Format ["%1MHQ = _MHQ; %1MHQDeployed = false; publicVariable '%1MHQ'; publicVariable '%1MHQDeployed';",str _side];
	//ok _MHQ setVehicleInit Format["[this, %1] execVM 'Server\Functions\MHQ_Wreck.sqf';",_side];
	//ProcessInitCommands;
	[[[_MHQ, _side], "Server\Functions\MHQ_Wreck.sqf"], "BIS_fnc_execVM", true, true] call BIS_fnc_MP;
