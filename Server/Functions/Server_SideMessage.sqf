Private ["_canSpeak","_extra","_localizedString","_locRaw","_receiver","_rlName","_side","_speaker","_special","_specialb","_topicSide","_value","_value2","_value3"];
_value2 = "";
_value3 = "";
...
if (_special in ["Destroyed","IsUnderAttack","Constructed"]) then {
	...
	switch (typeName _specialb) do {
		...
		case "STRING": {
			...
			if (_specialb == WESTCC || _specialb == EASTCC) then {_localizedString = localize "STR_WF_CommandCenter";_value = "UAVTerminal";if (WF_A2_Arrowhead || (WF_A2_CombinedOps && _side == west)) then {_value = "CommandPost"};};
			...
			if (_specialb == WESTFH || _specialb == EASTFH) then {_localizedString = localize "STRFieldHospital";_value = "FieldHospital"};
		};
	};
...
if (_special in ["VotingForNewCommander","NewIntelAvailable"]) then {
