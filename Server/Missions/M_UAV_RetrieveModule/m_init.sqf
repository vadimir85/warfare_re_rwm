...
if (count _randomPos == 0) exitWith {
	diag_log Format["[WFRE (INFORMATION)][frameno:%1 | ticktime:%2] M_UAV_RetrieveModule: Unable to determine a spawn location.",diag_frameno,diag_tickTime];
...
if (_eastUAV == '' && _westUAV == '') exitWith {
	diag_log Format["[WFRE (INFORMATION)][frameno:%1 | ticktime:%2] M_UAV_RetrieveModule: No suitable UAV Models defined for WFBE_EASTUAV & WFBE_WESTUAV.",diag_frameno,diag_tickTime];
	['_WFBE_M_RUNNINGMISSIONS_WEST',('_WFBE_M_RUNNINGMISSIONS_WEST' Call GetNamespace) - 1, true] Call SetNamespace;
	['_WFBE_M_RUNNINGMISSIONS_EAST',('_WFBE_M_RUNNINGMISSIONS_EAST' Call GetNamespace) - 1, true] Call SetNamespace;
};
...
_wteam = [];
_eteam = [];
_wteam = ['rhsusf_army_ucp_officer','rhsusf_army_ucp_rifleman','rhsusf_army_ucp_rifleman','rhsusf_army_ucp_rifleman_m590'];
_eteam = ['rhs_msv_sergeant','rhs_msv_grenadier','rhs_msv_rifleman','rhs_msv_rifleman'];
...
WFBE_LocalizeMessage = [_winner,'CLTFNCLOCALIZEMESSAGE',['SecondaryAward','$',20000]];
publicVariable 'WFBE_LocalizeMessage';
if (isHostedServer) then {[_winner,'CLTFNCLOCALIZEMESSAGE',['SecondaryAward','$',20000]] Spawn HandlePVF};
