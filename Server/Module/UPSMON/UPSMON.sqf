...
//		Comunidad Hispana de SimulaciÛn: 
//
...
// track unit ======================================================================================
	...
	if (_track=="TRACK") then {
		...
		_markertype = if (isClass(configFile >> "cfgMarkers" >> "mil_dot")) then {"mil_dot"} else {"mil_dot"};
		...
		_markerobj setMarkerShape "ICON";
		_markertype = if (isClass(configFile >> "cfgMarkers" >> "mil_flag")) then {"mil_flag"} else {"mil_flag"};
...
// TBD: add to global side arrays?
	...
	// create the clones
		for "_grpcnt" from 1 to _copies do {
			...
			//Ó¯Ë·Í‡ _lead setVehicleInit _initstr;
			_lead + ' ' + _initstr;//‡Ì‡ÎÓ„
			[_lead] join _grp;
			...
			_i=0;
			{
				...
				if (_i>1) then {
					...
					//Ó¯Ë·Í‡ _newunit setVehicleInit _initstr;
					_newunit + ' ' + _initstr;//‡Ì‡ÎÓ„
					[_newunit] join _grp;
				};
			} foreach _members;
...
// **********************************************************************************************************************
		...
		if (_pursue) then {		
			...
			//La distancia de flanqueo no puede ser superior a la distancia del objetivo o nos pordrÌa pillar por la espalda
			...
			//Calculamos posiciÛn de avance frontal			
			...
			//La preferencia es la elecciÛn inicial de direcciÛn
			switch (_flankdir) do {
				case 1: 
					{_prov = 125};
				case 2: 
			...
			//Evaluamos la direcciÛn en base a la provablilidad calculada
			...
			//PosiciÛn de ataque por defecto el flanco
			...
			if (_dist <= _closeenough) then {
				...
				if (( _dist <= (_closeenough + KRON_UPS_safedist))) then {
					...
					if ((_nomove=="NOMOVE" && _rnd < 75)  && !_reinforcementsent) then {
						...
						//Movimiento con precauciÛn (m·s r·pido)				
						_Behaviour =  "AWARE"; 		
						_wptype = "MOVE";						
					};										
				} else {
