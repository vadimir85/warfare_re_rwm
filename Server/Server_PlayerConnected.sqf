...
diag_log Format["[WFRE (INFORMATION)][frameno:%3 | ticktime:%4] Server_PlayerConnected: Player %1 (%2) has joined the game",_name,_uid,diag_frameno,diag_tickTime];
...
_get = Format["WFBE_JIP_USER%1",_uid] Call GetNamespace; //error
//[[GetNamespace], {_get = [Format["WFBE_JIP_USER%1",_uid]]}] call BIS_fnc_call; //fix
//_get = Format["WFBE_JIP_USER%1",_uid] remoteExecCall ["GetNamespace"];
...
if !(paramKeepAI) then {
	diag_log Format ["[WFBE (INFORMATION)][frameno:%7 | ticktime:%8] Server_PlayerConnected: Removing units (Team: %1, Team Side: %2, Team Leader: %3) for Player %4 (%5)   DEBUG:%6.",_team,side _team,name (leader _team),_name,_uid,_uid,diag_frameno,diag_tickTime];
	_units = units _team;
...
if (isNil '_get') exitWith {
	...
	*/
	[Format["WFBE_JIP_USER%1",_uid],[_uid,_slotIndex,5001,_sideJoined,_sideJoined,if (mysql) then {round(time)} else {0},vehicleVarName (leader _team)],true] Call SetNamespace;
...
//--- Update.
[Format["WFBE_JIP_USER%1",_uid],_get,true] Call SetNamespace; //error
//[[SetNamespace], {_get = [Format["WFBE_JIP_USER%1",_uid],true]}] call BIS_fnc_call; //fix
//[[Format["WFBE_JIP_USER%1",_uid]],true] remoteExecCall ["SetNamespace"];

//--- Did the player teamswap?.
if (_sideOrigin != _sideJoined) then {
	_funds = Format ["WFBE_%1STARTINGMONEY",str _sideJoined] Call GetNamespace; //error
	diag_log format ["STARTINGMONEY %1 != %2", _sideOrigin,_sideJoined];
	//[[GetNamespace], {_funds = Format ["WFBE_%1STARTINGMONEY",str _sideJoined]}] call BIS_fnc_call; //fix
	//_funds = Format["WFBE_%1STARTINGMONEY",str _sideJoined] remoteExecCall ["GetNamespace"];
};
