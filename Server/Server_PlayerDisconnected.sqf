...
GetNamespace = _this select 2;

diag_log Format["[UWFRE (INFORMATION)][frameno:%3 | ticktime:%4] Server_PlayerDisconnected: Player %1 (%2) has left the game",_name,_uid,diag_frameno,diag_tickTime];

//--- Grab the player info (if they exist).
_get = Format["WFBE_JIP_USER%1",_uid] Call GetNamespace;
if (isNil '_get') exitWith {diag_log Format ["[UWFRE (INFORMATION)][frameno:%3 | ticktime:%4] Server_PlayerDisconnected: Player %1 (%2) don't have any information stored.",_name,_uid,diag_frameno,diag_tickTime]};
...
if (isNull _team) exitWith {diag_log Format ["[UWFRE (INFORMATION)][frameno:%3 | ticktime:%4] Server_PlayerDisconnected: Player %1 (%2) team is null.",_name,_uid,diag_frameno,diag_tickTime]};
...
//--- Keep AI over JIP?
if !(paramKeepAI) then {
	diag_log Format ["[UWFRE (INFORMATION)][frameno:%7 | ticktime:%8] Server_PlayerDisconnected: Removing units (Team: %1, Team Side: %2, Team Leader: %3) for Player %4 (%5)   DEBUG:%6.",_team,side _team,leader _team,_name,_uid,_get,diag_frameno,diag_tickTime];
	_units = units _team;
...
//--- SetPos the AI.
if (side leader _team == east) then {_respawnLoc = "EastTempRespawnMarker"} else {_respawnLoc = "WestTempRespawnMarker"};
(leader _team) setpos getMarkerPos _respawnLoc;
...
if !(isNull (_cmdrTeam)) then {
	if (_team == _cmdrTeam) then {
		...
		//WFBE_LocalizeMessage = [_side,'CLTFNCLOCALIZEMESSAGE',['CommanderDisconnected']]; //not working in arma3?!
		WFBE_LocalizeMessage = ['CommanderDisconnected',_side] Call CLTFNCLOCALIZEMESSAGE; //fix
		...
		sleep 120;
		//--- Voting for new commander in 2 minutes
		if (isNull((_side) Call GetCommanderTeam)) then {
			WFBE_RequestCommanderVote = ['SRVFNCREQUESTCOMMANDERVOTE',_side];
			publicVariable 'WFBE_RequestCommanderVote';
			['SRVFNCREQUESTCOMMANDERVOTE',_side] Spawn HandleSPVF;
		};
	};
};